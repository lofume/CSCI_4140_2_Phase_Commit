...
This program uses many functions emulate a multi-company service
 ...
 purchase order and parts list based on the outputted data
*/
...
public class A3 {
    public static void main(String[] args) throws Exception {
        ...
        String URL = "jdbc:mysql://localhost:3306/A3Database";
        String USER = "root";
    ...
    static int loginIn568() {
        Scanner loginValue = new Scanner(System.in);
        while (true) {
            System.out.println("Please select the company you wish to browse:");
    ...
    static ResultSet Z_listParts568(Connection con568) {
        try {
            Statement st = con568.createStatement();
            return st.executeQuery("SELECT * FROM a3Database.`y-parts568` UNION SELECT * FROM a3Database.`x-parts568` " +
                    "where partDescription568 NOT IN (SELECT partDescription568 FROM a3Database.`y-parts568`)");
        } catch (Exception ex) {
            System.err.println(ex);
        }
        return null;
    ...
    static void Z_submitPO568(Connection con568, int clientNo568, ArrayList<Integer> partID568, ArrayList<Integer> partAmt568) {
        try {
            ...
            for (int i = 0; i < partID568.size(); i++) {
                String PartDesc;
                ResultSet reference = st.executeQuery("SELECT * FROM a3Database.`y-parts568` UNION SELECT * FROM " +
                        "a3Database.`x-parts568` WHERE partDescription568 NOT IN (SELECT partDescription568 FROM a3Database.`y-parts568`)");
